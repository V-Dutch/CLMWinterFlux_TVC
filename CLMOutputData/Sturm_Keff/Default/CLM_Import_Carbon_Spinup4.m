...
%% Import CLM Output
cd 'D:\PhD Work\Modelling\CLM\Output Data From CLM\Paper 2\GitRepo\TVC\MonteCarlo\ShoulderSeasonInvestigations\Sturm_Keff\1500Spinup_CryoT' % folder for model run, needs changing every time
%% Import Snow Parameters
% Hourly Snow Parameters:
SnowDepth_CLM = ncread('Restart_1500CryoT_STURM.clm2.h0.2013-01-01-00000.nc','SNOWDP');
Snowfall_CLM = ncread('Restart_1500CryoT_STURM.clm2.h0.2013-01-01-00000.nc','SNOW');
SWE_CLM = ncread('Restart_1500CryoT_STURM.clm2.h0.2013-01-01-00000.nc','H2OSNO'); 
SnowCoverFraction_CLM = ncread('Restart_1500CryoT_STURM.clm2.h0.2013-01-01-00000.nc','FSNO');
...
SnowLayerPresence = ncread('Restart_1500CryoT_STURM.clm2.h4.2013-01-01-00000.nc', 'SNO_EXISTENCE'); 
% 1 = Layer Present, 0 = Layer absent, decimal = proportion of hourly timestep layer present for 
SnowLayerDensities = ncread('Restart_1500CryoT_STURM.clm2.h4.2013-01-01-00000.nc','SNO_RHO'); %Only in runs 4 and onward
SnowLayerGrainRadii = ncread('Restart_1500CryoT_STURM.clm2.h4.2013-01-01-00000.nc','SNO_GS'); %Only in runs 6 and onward
SnowThermalConductivities = ncread('Restart_1500CryoT_STURM.clm2.h4.2013-01-01-00000.nc','SNO_TK');
SnowLayerThickness = ncread('Restart_1500CryoT_STURM.clm2.h4.2013-01-01-00000.nc','SNO_Z');
SnowLayerTemps = ncread('Restart_1500CryoT_STURM.clm2.h4.2013-01-01-00000.nc','SNO_T');
SnowLayerTemps = SnowLayerTemps - 273.15; % convert units
SnowIceContent = ncread('Restart_1500CryoT_STURM.clm2.h4.2013-01-01-00000.nc','SNO_ICE'); % hoping this will help explain density/Keff relationship
SnowLiquidWater = ncread('Restart_1500CryoT_STURM.clm2.h4.2013-01-01-00000.nc','SNO_LIQH2O'); % hoping this will help explain density/Keff relationship
...
SoilTemp_10cm_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','TSOI_10CM');
...
SoilLayerTemps_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','TSOI');
...
SoilMoisture_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','H2OSOI');
SoilMoisture_CLM = reshape(SoilMoisture_CLM,20,[]); %one row per layer

SoilLiquidWater_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','SOILLIQ');
SoilLiquidWater_CLM = reshape(SoilLiquidWater_CLM,20,[]); %one row per layer

PotentialSoilMoisture_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','SOILPSI');
PotentialSoilMoisture_CLM = reshape(PotentialSoilMoisture_CLM,25,[]); %one row per layer

SoilIce_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','SOILICE');
SoilIce_CLM = reshape(SoilIce_CLM,20,[]); %one row per layer

% Import soil properties to troubleshoot soil moisture:
% WetSoilDensity_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','soil_bulkdensity');
% WetSoilDensity_CLM = reshape(WetSoilDensity_CLM,25,[]); %one row per layer
% DrySoilDensity_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','dry_soildensity');
% DrySoilDensity_CLM = reshape(DrySoilDensity_CLM,25,[]); %one row per layer
% EffectiveSoilPorosity_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','EFF_POROSITY');
...
CO2Flux_CLM = ncread('Restart_1500CryoT_STURM.clm2.h2.2013-01-01-00000.nc','FCO2'); % Includes fires etc not in NEE
CH4Flux_CLM = ncread('Restart_1500CryoT_STURM.clm2.h2.2013-01-01-00000.nc','FCH4');

NEE_CLM = ncread('Restart_1500CryoT_STURM.clm2.h2.2013-01-01-00000.nc','NEE'); % NEE = NPP - ER
GPP_CLM = ncread('Restart_1500CryoT_STURM.clm2.h2.2013-01-01-00000.nc','GPP');
NPP_CLM = ncread('Restart_1500CryoT_STURM.clm2.h2.2013-01-01-00000.nc','NPP');
ER_CLM = ncread('Restart_1500CryoT_STURM.clm2.h2.2013-01-01-00000.nc','ER');
SoilResp_CLM = ncread('Restart_1500CryoT_STURM.clm2.h1.2013-01-01-00000.nc','SR'); 
...
AirTemp_CLM = ncread('Restart_1500CryoT_STURM.clm2.h3.2013-01-01-00000.nc','TBOT');
AirTemp_CLM = AirTemp_CLM - 273.15; % convert units
Rainfall_CLM = ncread('Restart_1500CryoT_STURM.clm2.h3.2013-01-01-00000.nc','RAIN');
RelativeHumidity_CLM = ncread('Restart_1500CryoT_STURM.clm2.h3.2013-01-01-00000.nc','RH');
Shortwave_CLM = ncread('Restart_1500CryoT_STURM.clm2.h3.2013-01-01-00000.nc','FLDS');
Longwave_CLM = ncread('Restart_1500CryoT_STURM.clm2.h3.2013-01-01-00000.nc','FSDS');
Pressure_CLM = ncread('Restart_1500CryoT_STURM.clm2.h3.2013-01-01-00000.nc','PBOT');
Pressure_CLM = Pressure_CLM/1000; % convert to KPa to match Obs
Windspeed_CLM = ncread('Restart_1500CryoT_STURM.clm2.h3.2013-01-01-00000.nc','WIND');
...
%% Construct Timetable
% Hourly Parameters:
Sturm_1500CT_Hourly = timetable(RawObs_Hourly.TIMESTAMP,RawObs_Hourly{:,6},AirTemp_CLM,RawObs_Hourly{:,7},RelativeHumidity_CLM,...
    ...
    SoilTemp_40cm_CLM,SoilTemp_58cm_CLM, SoilTemp_80cm_CLM, SoilTemp_106cm_CLM, SoilTemp_136cm_CLM,SoilTemp_170cm_CLM,SoilTemp_208cm_CLM,SoilTemp_250cm_CLM,....
    SoilTemp_299cm_CLM,SoilTemp_358cm_CLM,SoilTemp_427cm_CLM,SoilTemp_506cm_CLM, SoilTemp_595cm_CLM, SoilTemp_694cm_CLM, SoilTemp_803cm_CLM,...% "Deep" CLM Soil temps
...
% rename columns
Sturm_1500CT_Hourly.Properties.VariableNames{1} = 'AirTemp_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{2} = 'AirTemp_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{3} = 'RH_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{4} = 'RH_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{5} = 'Shortwave_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{6} = 'Shortwave_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{7} = 'Longwave_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{8} = 'Longwave_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{9} = 'AirPressure_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{10} = 'AirPressure_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{11} = 'Rainfall_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{12} = 'Rainfall_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{13} = 'Snowfall_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{14} = 'Snowfall_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{15} = 'SnowDepth_Echosounder_obs';
Sturm_1500CT_Hourly.Properties.VariableNames{16} = 'SnowDepth_MSC_obs';
Sturm_1500CT_Hourly.Properties.VariableNames{17} = 'SnowDepth_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{18} = 'SWE_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{19} = 'SCF_CLM';
% Want to splice in layer properties at this point
Sturm_1500CT_Hourly.Properties.VariableNames{20} = 'SoilTemp_2cm_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{21} = 'SoilTemp_5cm_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{22} = 'SoilTemp_10cm_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{23} = 'SoilTemp_20cm_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{24} = 'SoilTemp_1cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{25} = 'SoilTemp_4cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{26} = 'SoilTemp_9cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{27} = 'SoilTemp_10cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{28} = 'SoilTemp_16cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{29} = 'SoilTemp_26cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{30} = 'SoilTemp_40cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{31} = 'SoilTemp_58cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{32} = 'SoilTemp_80cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{33} = 'SoilTemp_106cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{34} = 'SoilTemp_136cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{35} = 'SoilTemp_170cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{36} = 'SoilTemp_208cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{37} = 'SoilTemp_250cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{38} = 'SoilTemp_299cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{39} = 'SoilTemp_358cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{40} = 'SoilTemp_427cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{41} = 'SoilTemp_506cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{42} = 'SoilTemp_595cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{43} = 'SoilTemp_694cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{44} = 'SoilTemp_803cm_CLM';

Sturm_1500CT_Hourly.Properties.VariableNames{45} = 'TotalSoilMoisture_1cm_CLM'; % m^3 m^-3
Sturm_1500CT_Hourly.Properties.VariableNames{46} = 'LiquidSoilMoisture_1cm_CLM'; % kg m^-3
Sturm_1500CT_Hourly.Properties.VariableNames{47} = 'SoilMoisture_2cm_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{48} = 'TotalSoilMoisture_4cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{49} = 'LiquidSoilMoisture_4cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{50} = 'SoilMoisture_5cm_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{51} = 'TotalSoilMoisture_9cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{52} = 'LiquidSoilMoisture_9cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{53} = 'SoilMoisture_10cm_Obs'; 
Sturm_1500CT_Hourly.Properties.VariableNames{54} = 'TotalSoilMoisture_16cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{55} = 'LiquidSoilMoisture_16cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{56} = 'SoilMoisture_20cm_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{57} = 'TotalSoilMoisture_26cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{58} = 'LiquidSoilMoisture_26cm_CLM';
...
Sturm_1500CT_Hourly.Properties.VariableNames{59} = 'SoilPSI_1cm_CLM'; %MPA
Sturm_1500CT_Hourly.Properties.VariableNames{60} = 'SoilPSI_4cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{61} = 'SoilPSI_9cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{62} = 'SoilPSI_16cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{63} = 'SoilPSI_26cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{64} = 'SoilPSI_40cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{65} = 'SoilPSI_58cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{66} = 'SoilPSI_80cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{67} = 'SoilPSI_106cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{68} = 'SoilPSI_136cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{69} = 'SoilPSI_170cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{70} = 'SoilPSI_208cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{71} = 'SoilPSI_250cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{72} = 'SoilPSI_299cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{73} = 'SoilPSI_358cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{74} = 'SoilPSI_427cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{75} = 'SoilPSI_506cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{76} = 'SoilPSI_595cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{77} = 'SoilPSI_694cm_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{78} = 'SoilPSI_803cm_CLM';

Sturm_1500CT_Hourly.Properties.VariableNames{79} = 'SoilIceContent_1cm_CLM'; %kg m^-3
Sturm_1500CT_Hourly.Properties.VariableNames{80} = 'SoilIceContent_4cm_CLM'; 
Sturm_1500CT_Hourly.Properties.VariableNames{81} = 'SoilIceContent_9cm_CLM'; 
Sturm_1500CT_Hourly.Properties.VariableNames{82} = 'SoilIceContent_16cm_CLM'; 
Sturm_1500CT_Hourly.Properties.VariableNames{83} = 'SoilIceContent_26cm_CLM'; 
...
Sturm_1500CT_Hourly.Properties.VariableNames{84} = 'NEE_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{85} = 'NEE_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{86} = 'NEE_CLM_gC';
Sturm_1500CT_Hourly.Properties.VariableNames{87} = 'CH4_Obs';
Sturm_1500CT_Hourly.Properties.VariableNames{88} = 'CH4_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{89} = 'NPP_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{90} = 'GPP_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{91} = 'EcosysResp_CLM';
Sturm_1500CT_Hourly.Properties.VariableNames{92} = 'CO2Flux_CLM'; 
Sturm_1500CT_Hourly.Properties.VariableNames{93} = 'SoilResp_CLM';

% export
cd 'D:\PhD Work\Modelling\CLM\Output Data From CLM\Paper 2\GitRepo\TVC\MonteCarlo\ShoulderSeasonInvestigations\Sturm_Keff\1500Spinup_CryoT'
writetimetable(Sturm_1500CT_Hourly,'1500CryoT_STURM_Hourly.csv');

% Daily Parameters:
Sturm_1500CT_Daily = retime(Sturm_1500CT_Hourly,'Daily');
Sturm_1500CT_Daily(end,:) = []; % creates empty row for first of Jan, remove it

Sturm_1500CT_Daily = addvars(Sturm_1500CT_Daily,SnowLayerGrainRadii,'Before','SoilTemp_1cm_CLM');
Sturm_1500CT_Daily = addvars(Sturm_1500CT_Daily,SnowLayerDensities,'Before','SnowLayerGrainRadii');
Sturm_1500CT_Daily = addvars(Sturm_1500CT_Daily,SnowLayerThickness,'Before','SnowLayerDensities');
Sturm_1500CT_Daily = addvars(Sturm_1500CT_Daily,SnowThermalConductivities,'Before','SoilTemp_1cm_CLM');
Sturm_1500CT_Daily = addvars(Sturm_1500CT_Daily,SnowLayerTemps,'Before','SoilTemp_1cm_CLM');
Sturm_1500CT_Daily = addvars(Sturm_1500CT_Daily,SnowIceContent, 'Before','SoilTemp_1cm_CLM');
Sturm_1500CT_Daily = addvars(Sturm_1500CT_Daily,SnowLiquidWater,'Before','SoilTemp_1cm_CLM');

%export
writetimetable(Sturm_1500CT_Daily,'1500CryoT_STURM_Daily.csv');