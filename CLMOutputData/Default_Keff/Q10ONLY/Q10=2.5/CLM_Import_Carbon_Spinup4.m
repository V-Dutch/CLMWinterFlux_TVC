...
SnowDepth_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h0.2013-01-01-00000.nc','SNOWDP');
Snowfall_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h0.2013-01-01-00000.nc','SNOW');
SWE_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h0.2013-01-01-00000.nc','H2OSNO'); 
SnowCoverFraction_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h0.2013-01-01-00000.nc','FSNO');
...
SnowLayerPresence = ncread('Restart_1500CryoT_Q10-2.5.clm2.h4.2013-01-01-00000.nc', 'SNO_EXISTENCE'); 
% 1 = Layer Present, 0 = Layer absent, decimal = proportion of hourly timestep layer present for 
SnowLayerDensities = ncread('Restart_1500CryoT_Q10-2.5.clm2.h4.2013-01-01-00000.nc','SNO_RHO'); %Only in runs 4 and onward
SnowLayerGrainRadii = ncread('Restart_1500CryoT_Q10-2.5.clm2.h4.2013-01-01-00000.nc','SNO_GS'); %Only in runs 6 and onward
SnowThermalConductivities = ncread('Restart_1500CryoT_Q10-2.5.clm2.h4.2013-01-01-00000.nc','SNO_TK');
SnowLayerThickness = ncread('Restart_1500CryoT_Q10-2.5.clm2.h4.2013-01-01-00000.nc','SNO_Z');
SnowLayerTemps = ncread('Restart_1500CryoT_Q10-2.5.clm2.h4.2013-01-01-00000.nc','SNO_T');
SnowLayerTemps = SnowLayerTemps - 273.15; % convert units
SnowIceContent = ncread('Restart_1500CryoT_Q10-2.5.clm2.h4.2013-01-01-00000.nc','SNO_ICE'); % hoping this will help explain density/Keff relationship
SnowLiquidWater = ncread('Restart_1500CryoT_Q10-2.5.clm2.h4.2013-01-01-00000.nc','SNO_LIQH2O'); % hoping this will help explain density/Keff relationship
...
SoilTemp_10cm_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','TSOI_10CM');
...
SoilLayerTemps_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','TSOI');
...
SoilMoisture_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','H2OSOI');
SoilMoisture_CLM = reshape(SoilMoisture_CLM,20,[]); %one row per layer

SoilLiquidWater_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','SOILLIQ');
SoilLiquidWater_CLM = reshape(SoilLiquidWater_CLM,20,[]); %one row per layer

PotentialSoilMoisture_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','SOILPSI');
PotentialSoilMoisture_CLM = reshape(PotentialSoilMoisture_CLM,25,[]); %one row per layer

SoilIce_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','SOILICE');
SoilIce_CLM = reshape(SoilIce_CLM,20,[]); %one row per layer

% Import soil properties to troubleshoot soil moisture:
% WetSoilDensity_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','soil_bulkdensity');
% WetSoilDensity_CLM = reshape(WetSoilDensity_CLM,25,[]); %one row per layer
% DrySoilDensity_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','dry_soildensity');
% DrySoilDensity_CLM = reshape(DrySoilDensity_CLM,25,[]); %one row per layer
% EffectiveSoilPorosity_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','EFF_POROSITY');
...
CO2Flux_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h2.2013-01-01-00000.nc','FCO2'); % Includes fires etc not in NEE
CH4Flux_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h2.2013-01-01-00000.nc','FCH4');

NEE_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h2.2013-01-01-00000.nc','NEE'); % NEE = NPP - ER
GPP_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h2.2013-01-01-00000.nc','GPP');
NPP_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h2.2013-01-01-00000.nc','NPP');
ER_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h2.2013-01-01-00000.nc','ER');
SoilResp_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h1.2013-01-01-00000.nc','SR'); 
...
AirTemp_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h3.2013-01-01-00000.nc','TBOT');
AirTemp_CLM = AirTemp_CLM - 273.15; % convert units
Rainfall_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h3.2013-01-01-00000.nc','RAIN');
RelativeHumidity_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h3.2013-01-01-00000.nc','RH');
Shortwave_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h3.2013-01-01-00000.nc','FLDS');
Longwave_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h3.2013-01-01-00000.nc','FSDS');
Pressure_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h3.2013-01-01-00000.nc','PBOT');
Pressure_CLM = Pressure_CLM/1000; % convert to KPa to match Obs
Windspeed_CLM = ncread('Restart_1500CryoT_Q10-2.5.clm2.h3.2013-01-01-00000.nc','WIND');
...
%% Construct Timetable
% Hourly Parameters:
Spinup1500CT_Q1025_Hourly = timetable(RawObs_Hourly.TIMESTAMP,RawObs_Hourly{:,6},AirTemp_CLM,RawObs_Hourly{:,7},RelativeHumidity_CLM,...
...
% rename columns
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{1} = 'AirTemp_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{2} = 'AirTemp_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{3} = 'RH_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{4} = 'RH_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{5} = 'Shortwave_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{6} = 'Shortwave_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{7} = 'Longwave_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{8} = 'Longwave_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{9} = 'AirPressure_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{10} = 'AirPressure_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{11} = 'Rainfall_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{12} = 'Rainfall_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{13} = 'Snowfall_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{14} = 'Snowfall_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{15} = 'SnowDepth_Echosounder_obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{16} = 'SnowDepth_MSC_obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{17} = 'SnowDepth_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{18} = 'SWE_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{19} = 'SCF_CLM';
% Want to splice in layer properties at this point
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{20} = 'SoilTemp_2cm_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{21} = 'SoilTemp_5cm_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{22} = 'SoilTemp_10cm_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{23} = 'SoilTemp_20cm_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{24} = 'SoilTemp_1cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{25} = 'SoilTemp_4cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{26} = 'SoilTemp_9cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{27} = 'SoilTemp_10cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{28} = 'SoilTemp_16cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{29} = 'SoilTemp_26cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{30} = 'SoilTemp_40cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{31} = 'SoilTemp_80cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{32} = 'SoilTemp_136cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{33} = 'SoilTemp_170cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{34} = 'SoilTemp_208cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{35} = 'SoilTemp_250cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{36} = 'SoilTemp_299cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{37} = 'SoilTemp_358cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{38} = 'SoilTemp_427cm_CLM';

Spinup1500CT_Q1025_Hourly.Properties.VariableNames{39} = 'TotalSoilMoisture_1cm_CLM'; % m^3 m^-3
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{40} = 'LiquidSoilMoisture_1cm_CLM'; % kg m^-3
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{41} = 'SoilMoisture_2cm_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{42} = 'TotalSoilMoisture_4cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{43} = 'LiquidSoilMoisture_4cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{44} = 'SoilMoisture_5cm_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{45} = 'TotalSoilMoisture_9cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{46} = 'LiquidSoilMoisture_9cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{47} = 'SoilMoisture_10cm_Obs'; 
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{48} = 'TotalSoilMoisture_16cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{49} = 'LiquidSoilMoisture_16cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{50} = 'SoilMoisture_20cm_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{51} = 'TotalSoilMoisture_26cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{52} = 'LiquidSoilMoisture_26cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{53} = 'SoilPSI_1cm_CLM'; %MPA
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{54} = 'SoilPSI_4cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{55} = 'SoilPSI_9cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{56} = 'SoilPSI_16cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{57} = 'SoilPSI_26cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{58} = 'SoilPSI_40cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{59} = 'SoilPSI_58cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{60} = 'SoilPSI_80cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{61} = 'SoilPSI_106cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{62} = 'SoilPSI_136cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{63} = 'SoilPSI_170cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{64} = 'SoilPSI_208cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{65} = 'SoilPSI_250cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{66} = 'SoilPSI_299cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{67} = 'SoilPSI_358cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{68} = 'SoilPSI_427cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{69} = 'SoilPSI_506cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{70} = 'SoilPSI_595cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{71} = 'SoilPSI_694cm_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{72} = 'SoilPSI_803cm_CLM';

Spinup1500CT_Q1025_Hourly.Properties.VariableNames{73} = 'SoilIceContent_1cm_CLM'; %kg m^-3
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{74} = 'SoilIceContent_4cm_CLM'; 
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{75} = 'SoilIceContent_9cm_CLM'; 
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{76} = 'SoilIceContent_16cm_CLM'; 
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{77} = 'SoilIceContent_26cm_CLM'; 
...
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{78} = 'NEE_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{79} = 'NEE_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{80} = 'NEE_CLM_gC';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{81} = 'CH4_Obs';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{82} = 'CH4_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{83} = 'NPP_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{84} = 'GPP_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{85} = 'EcosysResp_CLM';
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{86} = 'CO2Flux_CLM'; 
Spinup1500CT_Q1025_Hourly.Properties.VariableNames{87} = 'SoilResp_CLM';
...
writetimetable(Spinup1500CT_Q1025_Hourly,'Spinup1500CryoT_Q1025_Hourly.csv');

% Daily Parameters:
Spinup1500CT_Q1025_Daily = retime(Spinup1500CT_Q1025_Hourly,'Daily');
Spinup1500CT_Q1025_Daily(end,:) = []; % creates empty row for first of Jan, remove it

Spinup1500CT_Q1025_Daily = addvars(Spinup1500CT_Q1025_Daily,SnowLayerGrainRadii,'Before','SoilTemp_1cm_CLM');
Spinup1500CT_Q1025_Daily = addvars(Spinup1500CT_Q1025_Daily,SnowLayerDensities,'Before','SnowLayerGrainRadii');
Spinup1500CT_Q1025_Daily = addvars(Spinup1500CT_Q1025_Daily,SnowLayerThickness,'Before','SnowLayerDensities');
Spinup1500CT_Q1025_Daily = addvars(Spinup1500CT_Q1025_Daily,SnowThermalConductivities,'Before','SoilTemp_1cm_CLM');
Spinup1500CT_Q1025_Daily = addvars(Spinup1500CT_Q1025_Daily,SnowLayerTemps,'Before','SoilTemp_1cm_CLM');
Spinup1500CT_Q1025_Daily = addvars(Spinup1500CT_Q1025_Daily,SnowIceContent, 'Before','SoilTemp_1cm_CLM');
Spinup1500CT_Q1025_Daily = addvars(Spinup1500CT_Q1025_Daily,SnowLiquidWater,'Before','SoilTemp_1cm_CLM');

%export
writetimetable(Spinup1500CT_Q1025_Daily,'Spinup1500CryoT_Q1025_Daily.csv');